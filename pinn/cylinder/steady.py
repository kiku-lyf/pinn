...
def getData():
    ...
    inlet_u = umax * inlet_y * (0.4 - inlet_y) / (0.2 ** 2)
...
class PINN:
    ...
    def pde_loss(self, xy):
        ...
        u_xy = grad(u.sum(), xy, create_graph=True)[0]
        u_x = u_xy[:, 0:1]
        u_y = u_xy[:, 1:2]
        ...
        v_xy = grad(v.sum(), xy, create_graph=True)[0]
        v_x = v_xy[:, 0:1]
        v_y = v_xy[:, 1:2]
        ...
        p_xy = grad(p.sum(), xy, create_graph=True)[0]
        p_x = p_xy[:, 0:1]
        p_y = p_xy[:, 1:2]
        ...
        f1 =  u * u_x + v * u_y + p_x - self.mu*(u_xx+u_yy)
        f2 =  u * v_x + v * v_y + p_y - self.mu*(v_xx+v_yy)
    ...
    def closure(self):
        ...
        #     grad_loss_out = torch.autograd.grad(outputs=mse_outlet, inputs=self.net.parameters(), retain_graph=True,
        #                                        create_graph=True)
        #     grad_loss_out_norm = torch.sqrt(sum(torch.sum(g ** 2) for g in grad_loss_out))
        #
        #     # 平衡权重
        #     lambda_pde = (grad_loss_pde_norm + grad_loss_bc_norm+grad_loss_out_norm) / grad_loss_pde_norm
        #     lambda_bc = (grad_loss_pde_norm + grad_loss_bc_norm+grad_loss_out_norm) / grad_loss_bc_norm
        #     lambda_out = (grad_loss_pde_norm + grad_loss_bc_norm+grad_loss_out_norm) / grad_loss_out_norm
        #
        #     # 总损失
        #     loss = lambda_pde * mse_pde + lambda_bc * mse_bc+lambda_out*mse_outlet
        #
        # else:
...
if __name__ == "__main__":
    ...
    torch.save(pinn.net.state_dict(), "steady001.pt")
    plotLoss(pinn.losses, "loss_curve_steady001.png", ["BC", "Outlet", "PDE"])