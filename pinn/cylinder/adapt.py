...
class Model:
  ...
  # 有正则
    ...
    def LBGFS_epoch_loss(self):
        ...
        self.losses["bc"].append(loss_label.detach().cpu().item())
        # self.losses["outlet"].append(mse_outlet.detach().cpu().item())
        self.losses["pde"].append(loss_equation.detach().cpu().item())

        loss = self.true_loss(loss_equation, loss_label)
        loss.backward()
        self.net.iter += 1
        print('Iter:', self.net.iter, 'Loss:', loss.item()  ,'Loss_pde:',loss_equation.item() ,'Loss_bc:', loss_label.item())
        return loss
    ...
    def run_AM_AW1(self):
        ...
        for move_count in range(AM_count):
            ...
            for i in pbar:
                ...
                self.losses["bc"].append(loss_label.detach().cpu().item())
                # self.losses["outlet"].append(mse_outlet.detach().cpu().item())
                self.losses["pde"].append(loss_e.detach().cpu().item())
                ...
                self.net.iter += 1
                pbar.set_postfix({'Iter': self.net.iter,
                                  'Loss': '{0:.2e}'.format(loss.item()) ,'Loss_pde': '{0:.2e}'.format(loss_e.item()) ,'Loss_bc': '{0:.2e}'.format(loss_label.item())
                                  })
...
def x_f_loss_fun(x, train_U):
    ...
    f_u = u * udx + v * udy + pdx - (udxx + udyy) /re
    f_v = u * vdx + v * vdy + pdy - (vdxx + vdyy) / re
    f_f = udx + vdy
...
def plotLoss(losses_dict, path, info=[ "BC", "PDE"]):
...
if __name__ == '__main__':
    ...
    plotLoss(model.losses, "loss_curve_Re1000.png", ["BC", "PDE"])